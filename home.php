<?php
/**
 * Archive / Blog News Index Template
 * 
 * @package Royesh
 * @version 1.0.0
 */
    ...
    <div class="max-w-[1150px] mx-auto relative z-10 text-center text-white">
        ...
        <h1 class="text-3xl md:text-5xl font-black leading-tight mb-6">
            ...
            if (is_category()) {
                ...
                esc_html_e('اخبار، مقالات و اطلاعیه‌ها', 'royesh');
            }
            ?>
        </h1>
...
<section class="w-full pt-12 pb-6 px-4 md:px-12">
    <div class="max-w-[1150px] mx-auto flex flex-col md:flex-row gap-6 justify-between items-center bg-white p-4 md:p-6 rounded-[24px] border border-[#EBE5D7]/50 shadow-sm">
        ...
        <div id="category-filters" class="flex flex-wrap gap-2.5 items-center relative">
            <a href="<?php echo esc_url(get_post_type_archive_link('post')); ?>" class="px-5 py-2 rounded-full text-xs font-bold <?php echo !is_category() ? 'bg-[#014235] text-white' : 'bg-[#FAF8F4] text-gray-700 hover:bg-[#E8E2D2]'; ?> transition-all">
                <?php esc_html_e('همه', 'royesh'); ?>
            </a>
            <?php foreach ($categories as $cat) : ?>
                <a href="<?php echo esc_url(get_category_link($cat->term_id)); ?>" class="px-5 py-2 rounded-full text-xs font-bold <?php echo (is_category($cat->term_id)) ? 'bg-[#014235] text-white' : 'bg-[#FAF8F4] text-gray-700 hover:bg-[#E8E2D2]'; ?> transition-all">
